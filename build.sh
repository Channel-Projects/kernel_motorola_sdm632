...
if [[ $1 = "-r" || $1 = "--regen" ]]; then
	...
	echo -e "\nRegenerado com sucesso config $DEFCONFIG"
	exit
fi
...
if [[ $1 = "-rf" || $1 = "--regen-full" ]]; then
	...
	echo -e "\nRegenerado com sucesso config completa $DEFCONFIG"
	exit
fi
...
if [ -f "$kernel" ]; then
	...
	elif ! git clone -q https://github.com/Channel-Projects/AnyKernel3 -b Supra; then
